...
{
    public class SetHddActivationSecretRequest
    {
        public readonly byte[] ActivationSecret;

        public const int SIZE = 0x10;

        public static SetHddActivationSecretRequest? Read(byte[] data)
        {
            try
            {
                return new SetHddActivationSecretRequest(data);
            }
            catch (Exception)
            {
                return null;
        ...
        private SetHddActivationSecretRequest(byte[] data)
        {

            using MemoryStream mstream = new(data);
            using BinaryReader reader = new(mstream);

            ActivationSecret = reader.ReadBytes(0x10);
        }
    }
